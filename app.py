...
# Controller login credentials remain the same for simplicity
CONTROLLER_USERNAME = "controller"
CONTROLLER_PASSWORD = "controller_pass_123"
CONTROLLER_DISPLAY_NAME = "KRS Chandel" # Display name for the dashboard

# Ensure you have set the DATABASE_URL environment variable for deployment
...
def get_db_connection():
    ...
    try:
        ...
        # Set the timezone to UTC for all database operations
...
def get_class_id_by_name(class_name):
    """Retrieves the ID of a class from the database by its name."""
...
def get_active_aih_session():
    """Checks for and returns the currently active session for the AIH class."""
    ...
    try:
        ...
        # Calculate remaining time and deactivate if expired
        ...
        session_dict['class_name'] = 'BA - AIH'
...
@app.route('/')
def home():
    """Redirects to the appropriate dashboard based on login status."""
...
def login():
    """Handles the controller login process."""
...
def controller_dashboard():
    ...
    active_session = get_active_aih_session()
    conn = get_db_connection()
    if not conn:
    ...
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    past_sessions = []
    ba_aih_class_id = get_class_id_by_name('BA - AIH')

    if ba_aih_class_id:
        try:
            # Fetch all past (inactive) sessions for the AIH class
            cur.execute(
    ...
    return render_template('admin_dashboard.html',
                           ...
                           ba_aih_class_id=ba_aih_class_id,
                           all_sessions=past_sessions)

@app.route('/start_session', methods=['POST'])
@controller_required
def start_session():
    """Starts a new attendance session for the AIH class."""
    if get_active_aih_session():
    ...
    try:
        class_id = get_class_id_by_name('BA - AIH')
        if not class_id:
            raise Exception("BA - AIH class not found.")
...
def mark_attendance():
    """Handles student attendance marking requests."""
    if request.method == 'POST':
        data = request.form
        ...
        cur = conn.cursor()
        try:
            # Normalize enrollment number to uppercase before querying
            enrollment_no_upper = data['enrollment_no'].upper()
            cur.execute("SELECT id FROM students WHERE enrollment_no = %s AND batch = 'BA'", (enrollment_no_upper,))
            student_result = cur.fetchone()
            if not student_result:
                return jsonify({"success": False, "message": "Enrollment number not found.", "category": "danger"}), 404
            student_id = student_result[0]

            # Verify the session is active and valid
            cur.execute(
    ...
    # --- GET: Display the student attendance page ---
    active_session = get_active_aih_session()
    geofence_data = {}
    ...
    if conn:
        try:
            cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
            cur.execute("SELECT geofence_lat, geofence_lon, geofence_radius FROM classes WHERE class_name = 'BA - AIH' LIMIT 1")
...
def api_get_student_name(enrollment_no):
    """API endpoint to fetch a student's name by their enrollment number."""
    conn = get_db_connection()
    if not conn:
    ...
    try:
        ...
        cur.execute("SELECT name FROM students WHERE enrollment_no = %s AND batch = 'BA'", (enrollment_no_upper,))
...
def attendance_report():
    """Generates and displays a detailed daily attendance report."""
    conn = get_db_connection()
    if not conn:
        flash("Database connection failed.", "danger")
        return redirect(url_for('controller_dashboard'))
    ...
    if not class_id:
        flash("Error: 'BA - AIH' class not found.", "danger")
    ...
    try:
        ...
        if all_sessions:
            # Create a date range from the first session to today
...
def delete_daily_attendance():
    """Deletes all attendance records for a specific day."""
    ...
    try:
        ...
        if session_ids_to_delete:
            # This will also cascade delete records in attendance_records and session_device_fingerprints
...
def export_attendance_csv():
    """Exports the complete attendance history to a CSV file."""
    conn = get_db_connection()
    if not conn:
    ...
    class_id = get_class_id_by_name('BA - AIH')
    if not class_id:
        flash("Error: Class 'BA - AIH' not found.", "danger")
        return redirect(url_for('attendance_report'))

    try:
        # Get all students and unique session dates
        ...
        return send_file(
            ...
            mimetype='text/csv',
            as_attachment=True,
            download_name=f'BA_AIH_Semester_Report.csv'
        )
...
def edit_attendance(session_id):
    """Renders the page for manually editing a past session's attendance."""
    conn = get_db_connection()
    if not conn:
        flash("Database connection failed.", "danger")
        return redirect(url_for('controller_dashboard'))
...
def api_get_session_students_for_edit(session_id):
    """API endpoint to get all students and their attendance status for a specific session."""
    conn = get_db_connection()
    if not conn:
...
def api_update_attendance_record():
    """API endpoint to manually mark a student as present or absent."""
    ...
    try:
        cur = conn.cursor()
        if data['is_present']:
            # Add a record if marking as present
            cur.execute(
            ...
            # Delete the record if marking as absent
            cur.execute(
...
if __name__ == '__main__':
    # Use port 5001 to avoid conflicts with other apps if running locally
